...
def registrar_cliente(cliente: ClienteCreate):
    try:
        ...
        VALUES (?, ?, ?, ?, ?);
        """
        params = (cliente.nombre, cliente.apellido, cliente.correo_electronico, cliente.nombre_usuario, cliente.contrasena)
        ejecutar_consulta(query, params)
...
def iniciar_sesion(login: LoginRequest, request: Request):
    try:
        query_cliente = """
        SELECT ClienteID FROM Clientes WHERE NombreUsuario = ? AND Contrasena = ?;
        """
        params_cliente = (login.nombre_usuario, login.contrasena)
        ...
        if resultado_cliente:
            ...
            usuario_actual["cliente_id"] = resultado_cliente[0][0]
        ...
        query_admin = """
        SELECT AdministradorID FROM Administradores WHERE NombreUsuario = ? AND Contrasena = ?;
        """
        params_admin = (login.nombre_usuario, login.contrasena)
...
# Dependencia para obtener el rol actual del usuario
...
def crear_producto(nombre: str = Form(...), precio: float = Form(...), stock: int = Form(...), imagen: Optional[UploadFile] = File(None)):
    try:
        # Verificar y crear la carpeta de imágenes si no existe
        ...
        if imagen:
            # Guardar la imagen
            filepath = f"images/{imagen.filename}"
        ...
        VALUES (?, ?, ?, ?);
        """
        params = (nombre, precio, stock, filepath)
        ejecutar_consulta(query, params)
        ...
        return Producto(id=producto_creado[0], nombre=producto_creado[1], precio=producto_creado[2], stock=producto_creado[3], imagen=producto_creado[4])
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def obtener_productos():
    ...
    lista_productos = [
        Producto(id=producto[0], nombre=producto[1], precio=producto[2], stock=producto[3], imagen=f"/images/{producto[4]}" if producto[4] else None)
...
def comprar_producto(compra: CompraRequest):
    try:
        ...
        WHERE ProductoID = ?;
        """
        params_actualizar_stock = (compra.cantidad, producto_id)
        ejecutar_consulta(query_actualizar_stock, params_actualizar_stock)
        ...
        query_registrar_venta = """
        INSERT INTO Ventas (ClienteID, NombreUsuario, NombreProducto, Cantidad, TotalCompra, FechaVenta)
        VALUES (?, ?, ?, ?, ?, GETDATE());
        """
        nombre_usuario = usuario_actual["nombre_usuario"]  # Suponiendo que 'usuario_actual' tiene el nombre de usuario
        params_registrar_venta = (cliente_id, nombre_usuario, nombre_producto, compra.cantidad, total_compra)
        ejecutar_consulta(query_registrar_venta, params_registrar_venta)
...
@app.delete("/pedido/{pedido_id}", response_model=DeleteResponse)
    ...
    try:
        ...
        with pyodbc.connect(conn_str) as conn:
            ...
            try:
                ...
                logging.info("Eliminación de Pedidos completada")
                ...
                logging.info("Inserción en PedidosCancelados completada")
                ...
                logging.info("Actualización de stock completada")
